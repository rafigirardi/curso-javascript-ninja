(function() {
    ...
    /* testando da forma que aprendi no google
    function isOperatorValid(operator) {
      var validOperators = ["+", "-", "*", "/", "%"];
      return validOperators.includes(operator) ? true : false; // ainda não haviamos aprendido assim
    }
    console.log(isOperatorValid("+"));
  
    //modo ensinado no curso:
    function isOperatorValid( operator ){
        return (operator === '+' || operator === '-'|| operator === '*' || operator === '/' || operator === '%');
    }
  */
    //modo 2 ensinado no curso
    //  function isOperatorValid( operator ){
    //     return operation[operator] !== undefined;
    // }
  
    //modo 3 ensinado no curso
    function isOperatorValid(operator) {
